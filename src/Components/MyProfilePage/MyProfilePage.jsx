            ...
            <div className="card w-full sm:w-96 bg-green-100 shadow-xl mx-auto">
                <figure className="px-6 sm:px-10 pt-6 sm:pt-10">
                    <img
                        src={user?.photoURL || 'https://i.pravatar.cc/150?u=a042581f4e29026704d'}
                        alt="User Avatar"
                        className="rounded-xl h-32 w-32 sm:h-50 sm:w-50 object-cover"
                    />
                </figure>
                <div className="card-body items-center text-center space-y-4">
                    {isEditing ? (
                        <form onSubmit={handleUpdateProfile} className="w-full space-y-4">
                            <div>
                                <label className="label">
                                    <span className="label-text font-bold text-sm sm:text-base">Name</span>
                                </label>
                                <input
                                    type="text"
                                    value={name}
                                    onChange={(e) => setName(e.target.value)}
                                    className="input input-bordered w-full bg-green-300 text-sm sm:text-base"
                                    required
                                    disabled={isLoading}
                                />
                            </div>
                            <div>
                                <label className="label">
                                    <span className="label-text font-bold text-sm sm:text-base">Photo URL</span>
                                </label>
                                <input
                                    type="text"
                                    value={photoURL}
                            ...
                            <div className="card-actions justify-center space-x-2">
                                <button
                                    type="submit"
                                    className="btn bg-green-500 text-white hover:bg-green-600 border-green-600 hover:border-green-700 transition-all duration-300 transform hover:-translate-y-0.5 flex items-center gap-2 text-sm sm:text-base"
                                    disabled={isLoading}
                                >
                                    {isLoading ? 'Updating...' : (
                                        <>
                        ...
                        <>
                            <h2 className="card-title text-lg sm:text-xl">{user?.displayName || 'No Name Provided'}</h2>
                            <p className="text-sm sm:text-base">{user?.email || 'No Email Provided'}</p>
                            <div className="card-actions">
                                <button
